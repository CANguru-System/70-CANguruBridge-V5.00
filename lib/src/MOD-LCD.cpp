...
uint8_t amperes[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
//  display9341.fillScreen(ILI9341_BLACK);
  display9341.fillRect(0, 0, display9341.width(), display9341.height()*15/16, color);
...
//    display9341.drawCircle(middleX, middleY, i, color);
  ...
  display9341.setCursor(0, display9341.height()*15/16);

  int age = 23;

  sprintf(ampBuff, "1: %d.%d   2: %d.%d   3: %d.%d   4: %d.%d", amperes[0], amperes[1], amperes[2], amperes[3], amperes[4], amperes[5], amperes[6], amperes[7]);
  displayLCD(ampBuff);
  bfillRect = false;
}
